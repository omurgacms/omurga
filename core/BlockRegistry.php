...
final class Omurga_BlockRegistry
{
    ...
    private const CONTEXTS = ['home', 'page', 'post', 'archive', 'header', 'footer', 'sidebar', 'any'];
    ...
    public static function contextFromRegion(?string $region): ?string
    {
        ...
        if (in_array($region, ['home', 'home_top', 'home_main', 'homepage'], true)) {
            return 'home';
        }
        if (in_array($region, ['post', 'single', 'post_inside', 'single_post'], true)) {
            return 'post';
        }
        if (in_array($region, ['archive', 'category', 'tag', 'author'], true)) {
            return 'archive';
        }

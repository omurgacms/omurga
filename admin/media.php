    ...
    try{
        ...
        if($action==='upload'){
            ...
            for($i=0;$i<$count;$i++){
                ...
                $alt=trim($_POST['alt_text']??''); $abs=dirname(__DIR__).'/'.$finalPath; $info=is_file($abs)?omurga_image_info($abs):['width'=>0,'height'=>0]; $size=is_file($abs)?filesize($abs):0; $fmime=is_file($abs)?(mime_content_type($abs) ?: $mime):$mime;
                if(media_has_col_v34('width')){
                    db()->prepare("INSERT INTO $t (file_path,file_name,mime,alt_text,uploaded_by,original_path,width,height,file_size) VALUES (?,?,?,?,?,?,?,?,?)")->execute([$finalPath,basename($finalPath),$fmime,$alt,$_SESSION['omurga_user_id']??null,$originalPath===$finalPath?null:$originalPath,$info['width'],$info['height'],$size]);
                } else {
                    db()->prepare("INSERT INTO $t (file_path,file_name,mime,alt_text,uploaded_by) VALUES (?,?,?,?,?)")->execute([$finalPath,basename($finalPath),$fmime,$alt,$_SESSION['omurga_user_id']??null]);
                }
                $ok++;
            }

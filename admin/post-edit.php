...
if($id){
  ...
  if(defined('OMURGA_PAGE_EDITOR') && ($post['type'] ?? '') !== 'page'){ render_error_page(400,'Yanlış İçerik Türü','Bu ekran sadece sabit sayfalar içindir.'); }
...
if($_SERVER['REQUEST_METHOD']==='POST'){
  try{
    ...
    $featured = $isStaticPageSave ? '' : trim($_POST['featured_image'] ?? '');
    $social = '';
    if(!$isStaticPageSave && ($up=save_uploaded_file('featured_upload'))) { $featured=$up; insert_media_record($up, $title, $_SESSION['omurga_user_id']??null); }
    ...
    $commentsEnabled = !empty($_POST['comments_enabled']) ? 1 : 0;
    $newPostForRevision=[
    ...
    if($id){
      ...
      $data[]=$id; db()->prepare($sql)->execute($data); omurga_sync_post_categories($id, $isStaticPageSave ? [] : $categoryIds); sync_post_tags($id, $isStaticPageSave ? '' : ($_POST['tags'] ?? '')); if(!$isStaticPageSave) omurga_save_post_meta_values($id, $_POST['block_meta'] ?? []); if(!$isStaticPageSave){ omurga_set_post_meta($id, '_omurga_sidebar_enabled', isset($_POST['sidebar_enabled'])?'1':'0'); omurga_set_post_meta($id, '_omurga_sidebar_region', $_POST['sidebar_region']??'sidebar'); } else { omurga_set_post_meta($id, '_omurga_sidebar_enabled', isset($_POST['sidebar_enabled'])?'1':'0'); omurga_set_post_meta($id, '_omurga_sidebar_region', $_POST['sidebar_region']??'sidebar'); } omurga_do_action('omurga_after_post_save', $id, $newPostForRevision); if($status==='published') omurga_do_action('omurga_after_post_publish', $id, $newPostForRevision); log_activity('post.update','İçerik güncellendi: '.$title); echo '<div class="alert success">İçerik başarıyla güncellendi.</div>';
    } else {
      $sql="INSERT INTO $postsT (title,slug,spot,content,editor_type,content_blocks,type,status,category_id,featured_image,video_url,gallery_images,social_image,sort_order,seo_title,meta_description,focus_keyword,social_title,social_description,canonical_url,seo_noindex,comments_enabled,design_template,published_at,author_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
      $data[]=$_SESSION['omurga_user_id']??null; db()->prepare($sql)->execute($data); $id=(int)db()->lastInsertId(); omurga_sync_post_categories($id, $isStaticPageSave ? [] : $categoryIds); sync_post_tags($id, $isStaticPageSave ? '' : ($_POST['tags'] ?? '')); if(!$isStaticPageSave) omurga_save_post_meta_values($id, $_POST['block_meta'] ?? []); omurga_set_post_meta($id, '_omurga_sidebar_enabled', isset($_POST['sidebar_enabled'])?'1':'0'); omurga_set_post_meta($id, '_omurga_sidebar_region', $_POST['sidebar_region']??'sidebar'); omurga_do_action('omurga_after_post_save', $id, $newPostForRevision); if($status==='published') omurga_do_action('omurga_after_post_publish', $id, $newPostForRevision); log_activity('post.create','İçerik eklendi: '.$title); redirect('admin/'.($isStaticPageSave ? 'page-edit.php' : 'post-edit.php').'?id='.$id.'&saved=1');
    }
...
<div class="toolbar editor-head">
  <div><h1><?= $isStaticPage ? ($id?'Sabit Sayfa Düzenle':'Yeni Sabit Sayfa') : ((!$id && ($post['type']??'')==='news')?'Yazı Ekle':($id?e($currentLabel).' Düzenle':'Yeni '.e($currentLabel))) ?></h1><p><?= $isStaticPage ? 'WordPress mantığında sabit sayfa: kategori ve etiket kullanılmaz.' : 'Sade içerik girişi. Vitrin/tasarım alanları çekirdekte değildir; gerekirse tema, blok veya eklenti alanlarından gelir.' ?></p></div>
...
<form method="post" enctype="multipart/form-data" class="editor-layout"><input type="hidden" name="_csrf" value="<?=csrf_token()?>">
...
<div class="dle-editor-tabs" role="tablist"><button type="button" class="active">İçerik</button><button type="button">Ek Bilgiler</button><button type="button">Tasarım</button><button type="button">Erişim</button></div>
      <label>Kalıcı bağlantı / Slug<input id="slugInput" name="slug" value="<?=e($post['slug'])?>" placeholder="Boş bırakılırsa başlıktan oluşur"><small><?= $isStaticPage ? 'Sabit sayfa adresi kökten çalışır: /sayfa-adi' : 'Yazı adresi /yazi/yazi-adi yapısında çalışır.' ?></small></label>
  ...
  <aside class="editor-sidecol">
    ...
    <div class="side-box"><h3>Yorumlar</h3><label class="check-line"><input type="checkbox" name="comments_enabled" value="1" <?=om_post_comments_enabled($post)?'checked':''?>> <?=e(om_t('comments.allow_comments','Yorumlara izin ver'))?></label><small><?=e(om_t('comments.waiting_approval','Yeni yorumlar onay bekler.'))?></small></div>

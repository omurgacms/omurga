...
<form method="post" enctype="multipart/form-data" class="editor-layout" id="omContentEditForm"><input type="hidden" name="_csrf" value="<?=csrf_token()?>"><input type="hidden" name="post_id" value="<?=e($id)?>"><input type="hidden" name="autosave_draft_key" value="<?=e($autosaveDraftKey)?>">
  ...
  <aside class="editor-sidecol">
    <div class="side-box publish-box"><h3>Yayın</h3><label for="postStatusSelect">Durum</label><select id="postStatusSelect" name="status"><?php foreach(omurga_status_options_for_current_user($post['status'] ?? 'draft') as $sk=>$sv): ?><option value="<?=e($sk)?>" <?=($post['status']??'draft')===$sk?'selected':''?>><?=e($sv)?></option><?php endforeach; ?></select><label>Yayın Tarihi<input type="datetime-local" name="published_at" value="<?=e(omurga_datetime_local($post['published_at']??''))?>"></label><label>Sıralama<input type="number" name="sort_order" value="<?=e($post['sort_order'])?>"></label><button class="btn primary save-main" style="width:100%;justify-content:center"><?=can('posts.publish')?'Kaydet / Yayınla':'İncelemeye Gönder'?></button><button type="button" class="btn light" id="omEditorPreviewSideBtn" style="width:100%;justify-content:center;margin-top:8px">Editör Önizleme</button><?php if($id): ?><a class="btn light" style="width:100%;justify-content:center;margin-top:8px" target="_blank" href="<?=e($previewUrl)?>">Site Önizleme</a><?php endif; ?></div>
    ...
    <div class="side-box image-side"><h3>Öne Çıkan Görsel</h3>
      <input type="hidden" id="featuredImageInput" name="featured_image" value="<?=e($post['featured_image'])?>" data-preview="#featuredImagePreview">
      <div class="featured-image-actions">
        <button type="button" class="btn light" data-om-media-target="#featuredImageInput">Medyadan seç</button>
        <button type="button" class="btn light" data-om-clear-image="#featuredImageInput" data-preview="#featuredImagePreview">Kaldır</button>
      </div>
      ...
      <small>Öne çıkan görsel sadece Medya Kütüphanesi üzerinden seçilir. Bilgisayardan yükleme gerekiyorsa açılan medya penceresindeki yükleme alanını kullan.</small>
    </div>
...
<script src="../assets/js/omurga-editor.js?v=1.2.0-rc.1"></script>

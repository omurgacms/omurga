...
$groupLabels=['page'=>'Sabit Sayfa Şablonları','single'=>'Yazı Detay Şablonları'];
?>
...
<?php foreach($groupLabels as $group=>$label): ?>
  <section class="card template-group">
    <h2><?=e($label)?></h2>
    <p class="muted"><?= $group==='page' ? 'Hakkımızda, iletişim, hizmet ve özel sabit sayfalar için kullanılır.' : 'Yazı ve diğer içerik detayları için kullanılır.' ?></p>

...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    try{
        ...
        if($action==='upload'){
            ...
            log_activity('theme.upload','Tema yüklendi: '.($info['slug'] ?? ''));
            $msg='Tema yüklendi: '.($info['name'] ?? $info['slug']);
        }
        if($action==='delete'){
            $slug=preg_replace('/[^a-z0-9_-]/','',strtolower($_POST['theme_slug'] ?? ''));
...
<?php if($configuredTheme && $configuredTheme!==$active): ?><div class="alert pending">Kayıtlı aktif tema <b><?=e($configuredTheme)?></b> V4 standardıyla uyumlu değil veya eksik. Site geçici olarak <b><?=e($active)?></b> temasıyla çalışıyor.</div><?php endif; ?>
<section class="card">
...
<section class="card">
  <h2>Yeni Tema Yükle</h2>
  <p class="muted">Tema zip içinde <code>theme.json</code> olmalı. <code>template_engine: "omg"</code> için <code>home.omg/header.omg/footer.omg</code>, <code>template_engine: "php"</code> için <code>home.php/header.php/footer.php</code> kontrol edilir.</p>
  <form method="post" enctype="multipart/form-data"><input type="hidden" name="_csrf" value="<?=e(csrf_token())?>"><input type="hidden" name="action" value="upload"><input type="file" name="theme_zip" accept=".zip" required><button class="btn primary">Tema Zip Yükle</button></form>
</section>
<section class="card">
  <h2>Kolay Tema Yapısı / PHP Tema Yapısı</h2>
  <pre class="codebox">benim-omg-temam/
  theme.json  (template_engine: "omg")
  home.omg
  header.omg
  footer.omg
...
benim-php-temam/
  theme.json  (template_engine: "php")
  home.php
  header.php
  footer.php
  assets/css/style.css</pre>
</section>

...
$health=omurga_v23_system_health();
$errors=omurga_recent_error_lines(100);
?>
<h1>Sistem Sağlığı ve Güncellemeler</h1>
<?php if($msg): ?><div class="alert success"><?=e($msg)?></div><?php endif; ?>
<?php if($err): ?><div class="alert error"><?=e($err)?></div><?php endif; ?>

<div class="system-grid">
<section class="system-card"><h2>Sistem Sağlığı</h2><table class="status-table"><tbody><?php foreach($health as $row): ?><tr><td><?=e($row['name'])?></td><td><?=e($row['value'])?></td><td><span class="<?=$row['level']==='error'?'pill-bad':($row['level']==='warning'?'pill-info':'pill-ok')?>"><?=$row['level']==='error'?'Hata':($row['level']==='warning'?'Uyarı':'Sorunsuz')?></span></td></tr><?php endforeach; ?></tbody></table></section>

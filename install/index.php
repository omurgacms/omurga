...
$schema[]="CREATE TABLE IF NOT EXISTS {$p}post_meta (post_id INT UNSIGNED NOT NULL, meta_key VARCHAR(120) NOT NULL, meta_value MEDIUMTEXT NULL, PRIMARY KEY(post_id, meta_key), INDEX(meta_key)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4";
            ...
            $schema[]="CREATE TABLE IF NOT EXISTS {$p}activity_logs (id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY, user_id INT UNSIGNED NULL, action VARCHAR(80) NOT NULL, message TEXT NULL, ip VARCHAR(64) NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, INDEX(user_id), INDEX(action)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4";
            ...
            $stmt=$pdo->prepare("INSERT INTO {$p}users (name,email,username,password,role) VALUES (?,?,?,?, 'admin')");
            ...
            $pdo->prepare("INSERT INTO {$p}settings (setting_key, setting_value) VALUES (?,?) ON DUPLICATE KEY UPDATE setting_value=VALUES(setting_value)")->execute(['omurga_version','1.0.0-beta']);
            $pdo->prepare("INSERT INTO {$p}settings (setting_key, setting_value) VALUES (?,?) ON DUPLICATE KEY UPDATE setting_value=VALUES(setting_value)")->execute(['db_version','1.0.0-beta']);
            ...
            $profilePresets = [
                ...
                'kurumsal'=>[
                    ...
                    'theme'=>['primary_color'=>'#2563eb','secondary_color'=>'#0f172a','header_type'=>'classic','show_topbar'=>'1','footer_type'=>'dark','site_width'=>'wide','card_radius'=>'18','footer_text'=>'Omurga Kurumsal profili ile hazırlanmıştır.'],
                    'layout'=>'corporate'
                ],
                'topluluk'=>[
            ...
            $pdo->prepare("INSERT INTO {$p}settings (setting_key, setting_value) VALUES (?,?) ON DUPLICATE KEY UPDATE setting_value=VALUES(setting_value)")->execute(['theme_settings_omurga-kolay',json_encode($preset['theme'], JSON_UNESCAPED_UNICODE|JSON_PRETTY_PRINT)]);
            ...
            $demoSpot = 'Omurga CMS 1.0 Beta kurulumundan sonra gelen örnek başlangıç yazısı.';
            ...
            $commentStmt->execute([$demoPostId,'Omurga Ekibi','merhaba@omurgacms.com',$_SERVER['REMOTE_ADDR'] ?? '127.0.0.1','Omurga CMS 1.0 Beta sürümüne hoş geldiniz. Bu örnek yorum panelde yorum yönetimini test etmek için eklenmiştir.']);
  ...
  <aside class="install-side">
    <div>
      ...
      <div class="install-profile-mini">
        <div><b>Haber</b><span>DLE tarzı yayın</span></div>
...
<?php elseif($step===3): ?>
      <form method="post" class="install-form"><input type="hidden" name="step" value="4">
        ...
        <div class="type-grid">
          <label><input type="radio" name="site_type" value="haber" checked><span><b>Haber</b>DLE tarzı haber, kategori ve yayın akışı.</span><em class="profile-icon">📰</em></label>

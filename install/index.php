...
$schema[]="CREATE TABLE IF NOT EXISTS {$p}post_meta (post_id INT UNSIGNED NOT NULL, meta_key VARCHAR(120) NOT NULL, meta_value MEDIUMTEXT NULL, PRIMARY KEY(post_id, meta_key), INDEX(meta_key)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4";
            ...
            $pdo->prepare("INSERT INTO {$p}settings (setting_key, setting_value) VALUES (?,?) ON DUPLICATE KEY UPDATE setting_value=VALUES(setting_value)")->execute(['omurga_version','1.0.2-beta']);
            $pdo->prepare("INSERT INTO {$p}settings (setting_key, setting_value) VALUES (?,?) ON DUPLICATE KEY UPDATE setting_value=VALUES(setting_value)")->execute(['db_version','1.0.2-beta']);
            ...
            $demoSpot = 'Omurga CMS 1.0.2 Beta kurulumundan sonra gelen örnek başlangıç yazısı.';
            ...
            $commentStmt->execute([$demoPostId,'Omurga Ekibi','merhaba@omurgacms.com',$_SERVER['REMOTE_ADDR'] ?? '127.0.0.1','Omurga CMS 1.0.2 Beta sürümüne hoş geldiniz. Bu örnek yorum panelde yorum yönetimini test etmek için eklenmiştir.']);

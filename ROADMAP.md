# Omurga CMS Roadmap

## Omurga CMS 1.0.2 Beta

- [x] OMG template theme support
- [x] PHP theme support
- [x] Two default themes: Omurga Kolay and Omurga Sabit
- [x] Theme safety rules
- [x] Block system foundation
- [x] Package system foundation
- [x] Hook system foundation
- [x] Form builder beta
- [x] Sample post and sample comment after installation

## v1.1

- [ ] Media Library v2 improvements
- [ ] Better shared media picker modal
- [ ] Theme settings standard
- [ ] Package activation/deactivation improvements
- [ ] More developer examples

## v1.2

- [ ] Role and permission management improvements
- [ ] Import/export tools
- [ ] Theme/package marketplace foundation
- [ ] More default block examples

## v2.0 Stable

- [ ] Stable upgrade system
- [ ] Migration system
- [ ] Full developer documentation
- [ ] Production-ready release package

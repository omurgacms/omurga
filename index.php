...
"; }
    ...
    echo '<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">' . "
";
    foreach($posts as $p){ $date=substr($p['updated_at'] ?: $p['created_at'],0,10); echo '<url><loc>'.e(post_url($p)).'</loc><lastmod>'.e($date).'</lastmod><priority>0.8</priority></url>' . "
"; }
    echo '</urlset>'; exit;
}
if($slug==='' || $slug==='index.php'){
    ...
    if($st==='haber'){
                $latest=db()->query("SELECT p.*, c.name category_name, c.slug category_slug FROM $postsT p LEFT JOIN $catsT c ON c.id=p.category_id WHERE p.status='published' ORDER BY p.published_at DESC, p.id DESC LIMIT 18")->fetchAll();
        $featured=[];
        $popular=array_slice($latest,0,6);
        $catsForHome=db()->query("SELECT c.* FROM $catsT c WHERE EXISTS (SELECT 1 FROM $postsT p WHERE p.category_id=c.id AND p.status='published') ORDER BY c.sort_order ASC, c.id ASC LIMIT 6")->fetchAll();
        $categorySections=[];
        foreach($catsForHome as $cat){
            $cs=db()->prepare("SELECT p.*, c.name category_name, c.slug category_slug FROM $postsT p LEFT JOIN $catsT c ON c.id=p.category_id WHERE p.status='published' AND p.category_id=? ORDER BY p.published_at DESC, p.id DESC LIMIT 5");
            $cs->execute([$cat['id']]);
            $items=$cs->fetchAll();

# Omurga v4 Package API

Omurga v4 icin resmi paket klasoru `packages/` dizinidir. Eski `plugins/` klasoru geriye uyumluluk icin okunur; yeni gelistirme `packages/` icinde yapilmalidir.

## Zorunlu Yapi

```text
packages/ornek-paket/
  package.json
  package.php
  blocks/
    ornek-blok/
      block.json
      view.omg
```

## package.json

```json
{
  "name": "Ornek Paket",
  "slug": "ornek-paket",
  "version": "1.0.0",
  "description": "Omurga v4 paketi.",
  "author": "Omurga",
  "min_php": "8.1",
  "min_omurga": "4.0.0",
  "requires": [],
  "main": "package.php",
  "blocks": [],
  "admin_pages": []
}
```

## Etkinlestirme

Paketler `admin/packages.php` ekranindan etkinlestirilir veya devre disi birakilir. Gereksinimleri karsilanmayan paketler siteyi cokertmez; admin panelinde uyari gosterilir.

## Hook Kullanimi

`package.php` icinde hook ve blok API kullanilabilir:

```php
omurga_add_action('omurga_package_loaded', function ($slug, $meta) {
    // paket yuklendi
});
```
